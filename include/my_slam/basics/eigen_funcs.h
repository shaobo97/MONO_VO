...
// Get Affine3d using pos (x,y,z) and axis-rotation (rot_axis_x, rot_axis_y,
// rot_axis_z) with maginitude
Eigen::Affine3d getAffine3d(double x, double y, double z, double rot_axis_x,
                            double rot_axis_y, double rot_axis_z);

// -------------- CV <--> Eigen --------------

// Convert cv::Mat R and t --> Eigen Affine3d
Eigen::Affine3d transT_CVRt_to_EigenAffine3d(const cv::Mat &R,
                                             const cv::Mat &t);
